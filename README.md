...
| **模板控制** | 載入範本後,右側面板直接顯示 3–5 個以特效語言命名的巨集滑桿(火舌強度、破碎程度…),寫穿到底層節點 |
| 調參數(進階) | 點選節點進入該節點的完整參數;只露出關鍵參數,其餘收在「進階參數」摺疊內;取消選取回到模板控制 |
| 無縫檢查 | 預覽面板勾選「2×2 無縫檢查」 |
